...
public class CommandListener implements Listener {
    ...
    public static void onTabComplete(@NotNull PlayerChatTabCompleteEvent event) {
        if (!event.getPlayer().hasPermission("vulture.blocked.commands.bypass")) {
            event.getTabCompletions().removeIf(completion -> {
                for (@NotNull String command : Settings.blockedCommands) {
     ...
     */
    private static void processCommand(@NotNull Cancellable event, @NotNull CharSequence command, CommandSender sender) {
        // Check if the command is blocked.
        for (@NotNull String line : Settings.blockedCommands) {
            @NotNull Pattern pattern = Pattern.compile(line);
